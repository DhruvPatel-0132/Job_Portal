...
const SidebarProfile = ({ user }) => {
  // fallback user (only if zustand not loaded yet)
  const safeUser = user || {
    firstName: "Loading...",
    lastName: "",
    avatar: "/avatar.svg",
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
      <div className="flex justify-center pt-6 pb-2">
        <img
          src={safeUser.avatar || "/avatar.svg"}
          alt="Profile Avatar"
      ...
      <div className="text-center px-4 pb-4 border-b border-gray-100">
        <h2 className="text-base font-semibold text-gray-900 hover:underline cursor-pointer">
          {safeUser.firstName?.charAt(0).toUpperCase() +
            safeUser.firstName?.slice(1)}{" "}
          {safeUser.lastName?.charAt(0).toUpperCase() +
            safeUser.lastName?.slice(1)}
        </h2>
        <p className="text-xs text-gray-500 mt-1">{safeUser.headline || " "}</p>
      </div>
