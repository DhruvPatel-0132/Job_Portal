        ...
        {experience.map((exp, i) => (
          <motion.div key={i} className="relative pl-6 sm:pl-8 group">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-1 mb-2">
              <div>
                <h3 className="text-[17px] font-bold text-gray-900">{exp.title}</h3>
            ...
            {exp.skills && exp.skills.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2">
                {exp.skills.map((skill, idx) => (
                  <span key={idx} className="px-2.5 py-1 text-[12px] font-medium bg-gray-50 text-gray-600 rounded-lg border border-gray-200/80 hover:border-gray-300 transition-colors">
                    {skill}
                  </span>
                ))}
              </div>
